...
export default function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!rateCheck.allowed) {
      setRateLimitError(t.contact.rateLimitMsg.replace('{seconds}', String(rateCheck.timeLeft)))
            ...
            <form onSubmit={handleSubmit} className="space-y-4 max-w-md mx-auto">
              ...
              <div>
                <input
                  ...
                  onChange={handleChange}
                  placeholder={t.contact.telefono}
                  required
                  disabled={formStatus === 'loading'}
                  className={`w-full px-4 py-3 rounded-lg border transition-all
                    ${formErrors.phone
              ...
              <div>
                <select
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-gray-300
                           ...
                           disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <option value="general">{t.contact.servicio}</option>
                  <option value="diagnostico">{t.contact.diagnostico3d}</option>
                  <option value="marketing">{t.contact.marketingDigital}</option>
                  <option value="desarrollo">{t.contact.desarrolloWeb}</option>
                  <option value="consultoria">{t.contact.consultoria}</option>
                </select>
              </div>
                ...
                <div className="flex-1">
                  ...
                  {timeLeft > 0 && (
                    <p className="text-sm mt-1 text-amber-700">
                      {t.contact.tiempoRestante.replace('{seconds}', String(timeLeft))}
                    </p>
                  )}
                </div>
              </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h3 className="text-green-800 font-semibold text-lg">
                        {t.contact.exitoTitulo}
                      </h3>
                      <p className="text-green-700 mt-1">
                        {t.contact.exitoDesc}
                        <span className="font-semibold"> {t.contact.exitoHoras}</span>.
                      </p>
                      <p className="text-green-600 text-sm mt-2">
                        {t.contact.exitoRevisa}
                      </p>
                    </div>
                  </div>
                </div>
